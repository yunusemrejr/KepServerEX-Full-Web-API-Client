...
function normalAPI() {
	...
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint);
    fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint, {
            headers: {
    ...
    dropdownForChannels.addEventListener("change", function() {
        selectedTypeNameChannels = dropdownForChannels.value;
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels);

        fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels, {
                headers: {
                    Authorization: 'Basic ' + btoa(username + ':' + password),
                    'Content-Type': 'application/json'
                },
	...
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices");

        fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices", {
                headers: {
                    Authorization: 'Basic ' + btoa(username + ':' + password),
                    'Content-Type': 'application/json'
                },
	...
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC);

        fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC, {
                headers: {
                    Authorization: 'Basic ' + btoa(username + ':' + password),
                    'Content-Type': 'application/json'
                },
	...
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC + "/tags");


                fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC + "/tags", {
                        headers: {
                            Authorization: 'Basic ' + btoa(username + ':' + password),
                            'Content-Type': 'application/json'
                        },
	...
	console.log('fetch sending: '+'ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC + "/tags/" + selectedTypeNameTAG);
					
					
                    fetch('ServerRequests.php?kepurl='+normalApiConf + projectEndpoint + selectedTypeNameChannels + "/devices/" + selectedTypeNameDVC + "/tags/" + selectedTypeNameTAG, {
                            headers: {
                                Authorization: 'Basic ' + btoa(username + ':' + password),
                                'Content-Type': 'application/json'
                            },
...
function createNewTag(selectedTypeNameDVC, username, password) {
    ...
    console.log(path + " " + username + " " + password);
		...
		console.log('fetch sending: '+ 'ServerRequests.php?kepurl='+path);
	
    fetch('ServerRequests.php?kepurl='+path, {
            method: "POST",
            headers: {
                Authorization: 'Basic ' + btoa(username + ':' + password),
                "Content-Type": "application/json",
...
function updateTag(selectedTypeNameDVC, username, password, projectId) {
    ...
    let scanAddrsType = swal("Enter new tag values in order seperated by /: address / datatype / scan rate ",{
        content:"input",
    }).then((value)=>{
        return jsonNewValuesOfTag(value.toString());
    });
    
    tagJSONforChange=null;

    function jsonNewValuesOfTag(valuesConcated){
       const concattedvaluesarray = valuesConcated.split('/')
       const [tagaddress,tagdatatype,tagscanrate] = concattedvaluesarray.map(value => value.trim());
       ...
       tagJSONforChange = {
        ...
        afterInputGiven(tagJSONforChange);
}



    const afterInputGiven=(tagJSONforChange)=>{
        console.log(tagJSONforChange);
        let path =
        normalApiConf +
        projectEndpoint +
    ...
    fetch('ServerRequests.php?kepurl='+path, {
            ...
            body: JSON.stringify(tagJSONforChange),
        })
        .then((response) => {
            if (!response.ok) {
                console.log("HTTP error! Status: " + response.status);
...
function checkFixProjectID() {
    ...
    const displayedProjectId = projectId;
			...
			console.log('fetch sending: '+ 'ServerRequests.php?kepurl='+normalApiConf + '/config/v1/project');

	
    // Get the project ID from the API
    fetch('ServerRequests.php?kepurl='+normalApiConf + '/config/v1/project', {
            headers: {
                Authorization: 'Basic ' + btoa(username + ':' + password),
                'Content-Type': 'application/json'
            },
